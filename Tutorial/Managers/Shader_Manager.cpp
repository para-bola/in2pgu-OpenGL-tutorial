...
void Shader_Manager::CreateProgram(const std::string& shaderName,
	...
	//std::cout << "vertex_shader_code: \"" << vertex_shader_code << "\"" << std::endl;
	std::string fragment_shader_code = ReadShader(fragmentShaderFilename);
	//std::cout << "fragment_shader_code \"" << fragment_shader_code << "\"" << std::endl;
	GLuint vertex_shader = CreateShader(GL_VERTEX_SHADER, vertex_shader_code, "vertex shader");
	GLuint fragment_shader = CreateShader(GL_FRAGMENT_SHADER, fragment_shader_code, "fragment shader");

	int link_result = 0;
	// create program handle, attatch the shaders and link it
...
const GLuint Shader_Manager::GetShader(const std::string& shaderName){
	if (programs.find(shaderName) == programs.end()){
		// shader not found
		std::cout << "get: shader " << shaderName.c_str() << " not found!";
		return 0;
	}
	else {
...
void Shader_Manager::DeleteShader(const std::string& shaderName){
	if (programs.find(shaderName) == programs.end()) {
		// shader not found
		std::cout << "delete shader " << shaderName.c_str() << " not found!";
	}
	else {			
		glDeleteProgram(programs[shaderName]);
